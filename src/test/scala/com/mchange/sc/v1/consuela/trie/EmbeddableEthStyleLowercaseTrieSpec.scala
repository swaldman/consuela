...
class EmbeddableEthStyleLowercaseTrieSpec extends Specification with TestingUtilities { 

  private[this] implicit val logger = MLogger( this );

  def is =
s2"""
   An EmbeddableEthStyleLowercaseTrie can
...
""";
  ...
  val tt0 = new EmbeddableEthStyleLowercaseTrie();
 ...
 */
  ...
  def checkTrie( trie : EmbeddableEthStyleLowercaseTrie, yes : Set[String] ) : Boolean = {
  ...
  def checkEmpty( trie : EmbeddableEthStyleLowercaseTrie ) = tt3.RootHash == tt3.db.EmptyHash;
