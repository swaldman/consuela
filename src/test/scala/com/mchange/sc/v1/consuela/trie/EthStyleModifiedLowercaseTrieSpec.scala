...
class EthStyleModifiedLowercaseTrieSpec extends Specification { 

  private[this] implicit val logger = MLogger( this );

  def is =
s2"""
   An EthStyleModifiedLowercaseTrie can
...
""";
  ...
  val tt0 = new EthStyleModifiedLowercaseTrie();
  ...
  def checkTrie( trie : EthStyleModifiedLowercaseTrie, yes : Set[String] ) : Boolean = {
  ...
  def checkEmpty( trie : EthStyleModifiedLowercaseTrie ) = tt3.root == tt3.db.Zero;
