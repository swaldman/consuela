...
package object jsonrpc {
  ...
  def toErrorHexBytes( errorData : String ) : Failable[immutable.Seq[Byte]] = Failable.flatCreate {
    def giveUp( extra : String = "") = Failable.fail( s"(Error data '${errorData}' was not a decodable hex string.${extra})" )

    if ( errorData.isEmpty ) {
      Failable.fail( "(Error data was an empty string.)" )
    }
    else {
      Failable( errorData.decodeHexAsSeq ).recoverWith {
