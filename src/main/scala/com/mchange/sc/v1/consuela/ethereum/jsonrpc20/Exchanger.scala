...
object Exchanger {
  ...
  final object Play {
    ...
    object WSClient {
      ...
      def apply( url : String, clientConfig : ahc.AhcWSClientConfig /* = DefaultAhcClientConfig */, manager : Manager ) : WSClient = {
        manager.synchronized {
          val configBuilder = new ahc.AhcConfigBuilder( clientConfig )
          val out = new WSClient( url, new ahc.AhcWSClient( configBuilder.build() )( manager.materializer ), manager )
          manager.register( out )
          out
        }
      }
    }
    class WSClient( url : String, client : play.api.libs.ws.WSClient, manager : WSClient.Manager ) extends Exchanger {
